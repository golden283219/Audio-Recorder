package com.appbestsmile.voicelikeme.audiovisualization;
...
class GLWaveLayer {
  ...
  public GLWaveLayer(GLAudioVisualizationView.Configuration configuration, float[] color,
      float fromY, float toY, Random random) {
    this.configuration = configuration;
    this.random = random;
    this.waves = new GLWave[configuration.wavesCount];
    float footerToY = fromY
        + configuration.footerHeight / (configuration.footerHeight + configuration.waveHeight * 2)
        * (toY - fromY);
  ...
  private GLBubble[] generateBubbles(float[] color, int count) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      bubbles[i] =
          new GLBubble(col, -1 + random.nextFloat() * 2, bubbleFromY + shift, bubbleToY, size,
              random);
    }
    return bubbles;
  }
